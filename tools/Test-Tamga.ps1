...
Assert-Tamga ($scriptText -match 'elseif \(\$isWebResource\) \{ ''WEB'' \}') 'İnternet kaynağı rozeti WEB olmalı.'
Assert-Tamga ($scriptText -notmatch 'elseif \(\$isWebResource\) \{ ''SİTE'' \}') 'Eski SİTE kaynak rozeti kullanılmamalı.'
...
Assert-Tamga ($scriptText -match 'x:Name="AboutUpdatePanel"') 'Modern güncelleme durum kartı bulunamadı.'
Assert-Tamga ($scriptText -match 'x:Name="AboutUpdateProgress"') 'Güncelleme ilerleme göstergesi bulunamadı.'
Assert-Tamga ($scriptText -match 'x:Name="AboutRestartButton"') 'Güncelleme sonrası yeniden başlatma düğmesi bulunamadı.'
Assert-Tamga ($scriptText -match "ValidateSet\('Checking','Available','Current','Installing','Ready','Error'\)") 'Güncelleyici durum modeli eksik.'
Assert-Tamga ($scriptText -match 'function Start-TamgaSelfUpdate') 'Arka plan Tamga güncelleyicisi bulunamadı.'
Assert-Tamga ($scriptText -match '\-InstallDirectory \$installDirectory \-NoLaunch') 'Güncelleyici sessiz hazırlama modunu kullanmalı.'
Assert-Tamga ($scriptText -match '\-WindowStyle Hidden \-PassThru') 'Güncelleyici arka planda izlenebilir bir süreç başlatmalı.'
Assert-Tamga ($scriptText -match 'function Complete-TamgaSelfUpdate') 'Güncelleme sonucu doğrulaması bulunamadı.'
